...
void saxpyCuda(int N, float alpha, float *xarray, float *yarray, float *resultarray)
{
    ...
    double startTimeWithoutTransfer = CycleTimer::currentSeconds();
    saxpy_kernel<<<blocks, threadsPerBlock>>>(N, alpha, device_x, device_y, device_result);
    cudaDeviceSynchronize();
    double endTimeWithoutTransfer = CycleTimer::currentSeconds();
    ...
    overallDuration = endTimeWithoutTransfer - startTimeWithoutTransfer;
    printf("saxpy without data transfer: %.3f ms\n", 1000.f * overallDuration);

...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        "*** YOUR CODE HERE ***"
        if (exit and hasattr(exit, "entrance")):
            exit.entrance = self
        # END Problem 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        dist, a, b = 0, self.lower_bound, self.upper_bound
        while (not cur_place.is_hive):
            
            if (a <= dist <= b and len(cur_place.bees) != 0):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if (amount >= self.health):
            amount = self.health + self.damage

        if (len(self.place.bees) > 0):
            for bee in self.place.bees[:]:
                bee.reduce_health(amount)

        Ant.reduce_health(self, min(self.health, amount))
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        con1 = self.ant_contained == None
        con2 = not other.is_container
    ...
    def action(self, gamestate):
        ...
        while (cur_place != None):
            ...
            if (cur_place.ant != None):
                ...
                if (hasattr(cur_place.ant, "ant_contained")):
                    if (cur_place.ant.ant_contained != None):
                        cur_place.ant.ant_contained.double()
            cur_place = cur_place.exit

        target_bee = self.nearest_bee()
        if (target_bee != None):
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def new_action(gamestate):
            nonlocal target
            con = target.slowed > 0 and gamestate.time % 2 == 0 
            if (target.slowed <= 0 or con):
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        cur_place = self.place
        dis = 0
        while (cur_place != None and cur_place.is_hive == False):
            ant = cur_place.ant
            if (ant != None and ant.name != 'Laser'):
                res[ant] = dis
                if (hasattr(ant, "ant_contained")):
                    if (ant.ant_contained != None):
                        res[ant.ant_contained] = dis

            for bee in cur_place.bees:
                res[bee] = dis

            cur_place, dis = cur_place.entrance, dis + 1
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        reduce = distance * 0.25 + self.insects_shot * 0.0625
        return self.damage - reduce
        # END Problem Optional 2False   

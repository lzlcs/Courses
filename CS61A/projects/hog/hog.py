...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    tmp, delta = score + tail_points(opponent_score), 0
    if (perfect_square(tmp)):
        delta = next_perfect_square(tmp) - score
    else: 
        delta = tmp - score
    if (delta >= threshold):
        return 0

...
def add_trees(t1, t2):
    ...
    for i in range(len(b1)):
        if (i >= len(b2)):
            other += [add_trees(b1[i], tree(0))]
        else:
            other += [add_trees(b1[i], b2[i])]

    return tree(label(t1) + label(t2), other)
